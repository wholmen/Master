...
CCDIntermediates::CCDIntermediates(basis_set BASIS){
    ...
    Nholes = basis.Nholes; Nholes2 = Nholes*Nholes;
    Nstates = basis.Nstates;
    Nparticles = Nstates - Nholes; Nparticles2 = Nparticles*Nparticles;
...
double CCDIntermediates::CCD(int MaxIterations){
    ...
    cout << "Energy using intermediates: " << E0 / Nholes << endl;

    // Generate first set of new amplitudes and do second calculation
    UpdateAmplitudes();
    double E1 = CorrelationEnergy();

    cout << "Energy using intermediates: " << E1 / Nholes << endl;
                ...
                for (int l=0; l<Nholes; l++){
                    ...
                    for (int cc=0; cc<Nparticles; cc++){
                        for (int dd=0; dd<Nparticles; dd++){
                            ...
                            int c = cc + Nholes; int d = dd + Nholes;
                            I1(i+j*Nholes, k+l*Nholes) += basis.TwoBodyOperator(k,l,c,d) * t( Index(cc,dd,i,j,Nparticles,Nparticles,Nholes));
                        }
                    }
                ...
                for (int cc=0; cc<Nparticles; cc++){
                    ...
                    for (int l=0; l<Nholes; l++){
                        for (int dd=0; dd<Nparticles; dd++){
                            ...
                            int d = dd + Nholes;
                            I2(j+k*Nholes, bb+cc*Nparticles) += basis.TwoBodyOperator(k,l,c,d) * t( Index(dd,bb,l,j,Nparticles,Nparticles,Nholes));
                        }
                    }
            ...
            for (int l=0; l<Nholes; l++){
                for (int cc=0; cc<Nparticles; cc++){
                    for (int dd=0; dd<Nparticles; dd++){
                        ...
                        I3(j,k) += basis.TwoBodyOperator(k,l,c,d) * t( Index(cc,dd,j,l,Nparticles,Nparticles,Nholes));
                    }
                }
            }
        }
            ...
            for (int k=0; k<Nholes; k++){
                for (int l=0; l<Nholes; l++){
                    for (int dd=0; dd<Nparticles; dd++){
                        ...
                        I4(bb,cc) += basis.TwoBodyOperator(k,l,c,d) * t( Index(bb,dd,k,l,Nparticles,Nparticles,Nholes));
                    }
                }
            }
        }
